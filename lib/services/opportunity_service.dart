...
class OpportunityService {
  ...
  static Future<List<Opportunity>> fetchOpportunities() async {
    ...
    if (debugForceError) {
      throw OpportunityFetchException('Failed to load opportunities. Please try again.');
    }
  ...
  static Future<Opportunity> fetchOpportunityById(String id) async {
    ...
    if (debugForceError) {
      throw OpportunityFetchException('Failed to load program details. Please try again.');
    }
    
    return mockOpportunities.firstWhere(
      (opp) => opp.id == id,
      orElse: () => throw OpportunityFetchException('Program with ID $id not found.'),
    );
  }
}
